...
def test_fetch_patient_reads_from_snapshot(tmp_path: Path):
    ...
    # A complete snapshot load is NOT partial data, and its provenance ("loaded
    # from local snapshot") is informational only — it must NOT be surfaced as a
    # clinician-facing warning (warnings feed DecisionPacket.data_gaps).
...
def test_complete_snapshot_load_surfaces_no_partial_notice(tmp_path: Path):
    """A complete snapshot load must produce NO partial-data notice, so the
    informational provenance never reaches DecisionPacket.data_gaps as a fake
    clinical gap. (Bug: warnings=['loaded from local snapshot'] made
    partial_data_notice emit a 'Some data could not be retrieved' notice for a
    fully-complete load.)"""
    from backend.app.observability.warnings import partial_data_notice

    snap_bundle = {
        "resourceType": "Bundle",
        "type": "searchset",
        "entry": [
            {"resource": {"resourceType": "Patient", "id": "p-snap", "gender": "male"}},
        ],
    }
    snap = tmp_path / "snap.json"
    snap.write_text(json.dumps(snap_bundle))
    result = asyncio.run(MockFHIRProvider(snapshot_path=snap).fetch_patient("p-snap"))
