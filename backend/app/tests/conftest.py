...
@pytest.fixture(autouse=True)
def _offline_knowledge_evidence(monkeypatch):
    """Keep /packet offline by default.

    ``_build_packet_body`` computes live drug-knowledge evidence (RxNav/openFDA);
    stub it to an empty result so endpoint tests don't hit the network. Tests that
    exercise computed evidence override ``backend.app.api.packet.build_knowledge_evidence``
    themselves. The real ``build_knowledge_evidence`` is unit-tested directly in
    test_knowledge_evidence.py (this patch only affects the name imported into the
    packet API module).
    """
    import backend.app.api.packet as packet_mod
    from backend.app.reasoning.knowledge_evidence import KnowledgeEvidence

    async def _empty(_resources, _age_years):
        return KnowledgeEvidence()

    monkeypatch.setattr(packet_mod, "build_knowledge_evidence", _empty)
