...
def test_conditions_have_subject(result: FetchResult) -> None:
    # subject is required by R4B Condition — without it the resource is dropped at the
    # /intake validation boundary (FHIR-01/02). Every extracted Condition must carry it.
    conditions = _resources(result, "Condition")
    assert conditions
    for c in conditions:
        assert c["subject"]["reference"] == f"Patient/{_PATIENT_ID}"
