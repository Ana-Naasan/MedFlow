...
# ---------------------------------------------------------------------------
# delete_resource (#91: clear a stale partial-data notice when a source recovers)
# ---------------------------------------------------------------------------


def test_delete_resource_removes_row_then_reports_absence():
    async def run():
        engine, factory = await _make_session()
        async with factory() as sess:
            await repo.upsert_resource(
                sess,
                patient_id="p1",
                resource_type="FetchMetadata",
                resource_id="p1",
                body={"level": "warning"},
                source_provider="mock",
            )
            # Present → removed, returns True.
            deleted = await repo.delete_resource(
                sess, patient_id="p1", resource_type="FetchMetadata", resource_id="p1"
            )
            gone = await sess.get(CachedResource, ("p1", "FetchMetadata", "p1"))
            # Absent → returns False (idempotent clear, never raises).
            missing = await repo.delete_resource(
                sess, patient_id="p1", resource_type="FetchMetadata", resource_id="p1"
            )
        await engine.dispose()
        return deleted, gone, missing

    deleted, gone, missing = asyncio.run(run())
    assert deleted is True
    assert gone is None
    assert missing is False


def test_list_patient_resources_excludes_bookkeeping_and_other_patients():
    """list_patient_resources returns only a patient's real FHIR rows — never the
    synthetic FetchMetadata / DecisionPacket bookkeeping rows, and never another
    patient's resources (composite PK is patient-scoped)."""
    ...
    async def run():
        ...
        async with factory() as sess:
            await repo.upsert_resource(
                ...
                resource_id="pX",
                body={"resourceType": "Patient", "id": "pX"},
            ...
            await repo.upsert_resource(
                ...
                resource_id="m1",
                body={"resourceType": "MedicationStatement", "id": "m1"},
                source_provider="mock-fhir",
            )
            # Bookkeeping rows that must be excluded.
            await repo.upsert_resource(
                sess,
                patient_id="pX",
                resource_type="FetchMetadata",
            ...
            # A different patient's resource must not leak.
            await repo.upsert_resource(
                sess,
                patient_id="other",
                resource_type="Condition",
                resource_id="c1",
                body={"resourceType": "Condition", "id": "c1"},
                source_provider="mock-fhir",
            )
            rows = await repo.list_patient_resources(sess, patient_id="pX")
            keys = sorted((r.resource_type, r.resource_id) for r in rows)
        await engine.dispose()
        return keys

    keys = asyncio.run(run())
    assert keys == [("MedicationStatement", "m1"), ("Patient", "pX")]
