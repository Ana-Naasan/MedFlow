...
class MockFHIRProvider(Provider):
    ...
    def _load_snapshot(self, patient_id: str, path: Path | None = None) -> FetchResult:
        ...
        # A complete snapshot load is NOT partial data. The "loaded from local
        # snapshot" provenance is informational only and is conveyed by
        # ``source``; it must NOT be a surfaced warning, because warnings flow
        # into DecisionPacket.data_gaps and would mislead the clinician into
        # thinking real data was missing.
        return _make_result(
            bundle,
            patient_id,
            self.id,
