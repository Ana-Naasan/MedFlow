...
def live_gap_fhir_types(resources: list[dict]) -> set[str]:
    """Safety-critical FHIR types absent from a patient's record (coverage-free).

    The live reasoning path has no source coverage map, so derive the downgrade
    set straight from the record: a safety-critical category
    (AllergyIntolerance, Observation) with no ``present`` records is treated as
    a gap. This gives model-generated hypotheses the same "never claim safety
    from absence" downgrade the static scaffold applies (REASON-06 / #30).
    """
    return {
        fhir_type
        for fhir_type in _SAFETY_CRITICAL_FHIR_TYPES
        if coverage_state(fhir_type, list(resources)) != "present"
    }
