...
export function placeholderPatient(id: string): Patient {
  // A real backend patient (e.g. pat-001 / DEMO-001) isn't in the mock EMR. Return a
  // complete, honest placeholder so the /patients/[id] subtree renders (the packet page
  // loads the real backend data) instead of 404ing on an id the backend knows about.
  // Demographics are left blank — the header shows "—" rather than fabricating them.
  return {
    id,
    name: { first: "Backend", last: "Patient" },
    dob: "",
    sex: "other",
    location: "Loaded from backend",
    healthNumber: id,
    primaryPhysician: "—",
    allergies: [],
    medications: [],
...
export function resolvePatient(id: string): Patient {
  return getPatientById(id) ?? placeholderPatient(id);
}
