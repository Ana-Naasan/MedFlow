...
- **#59 (#27 PDF connector, feat/pdf-connector, Vivek)** — CONN-04 char-offset provenance (M3). RECONCILED locally (planning merged, lint fixed — WIP commit on `feat/pdf-connector`, NOT pushed) but NOT merged: it **fails the gate** (was red on CI too) — `pdf.py` 92% (12 uncovered lines) AND it emits **`MedicationRequest`**, which is NOT in the 6-resource subset (downstream flatten/reason expect **`MedicationStatement`**). Needs: resource-type remap → MedicationStatement (valid R4B) + tests to 100%. Off the critical path.
- **#63 (#33 observability, feat/observability-partial-warnings-33)** — structured logging + partial-data warning surfacing. M4/Phase-5. Not yet reviewed.
- **#62 (ci: Claude auto-review workflow, ci/claude-auto-review, Bader)** — adds the Claude GitHub Action; merge after Bader installs the Claude GitHub App + adds an auth secret (admin-only steps).
